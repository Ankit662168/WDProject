...
import Locator.SD_LoginPage.SDLogins_PageLocator;

public class SDLoginPage extends baseclass implements SDLogins_PageLocator {
...
public void SDENLogin(int en) throws Exception, InterruptedException, IOException {
	try {
		Excelreuse e = new Excelreuse();
		ArrayList datas = e.getData(prop.getProperty("Excelinputs"));
		driver.get((String) datas.get(en));
		...
		element.sendKeys((String) datas.get(4));
		...
		System.out.println("User is able to login to the EN customer portal successfully");		
